...
- 基于V8和V9RN模型的二维交叉分析（支持自定义模型名称）
- 双算法引擎：贪心矩形扩充 + 0-1整数线性规划（ILP）全局最优解
- 智能置入置出算法自动生成排除区域
- 新老策略全面对比评估
- 生成企业规范Excel分析报告 + ECharts交互式HTML报告
- 安全过件率阈值约束和交易占比约束
- 二分搜索自适应阈值调整
...
| `--model_x` | string | `V8` | X轴模型名称 |
| `--model_y` | string | `V9RN` | Y轴模型名称 |
| `--algorithm` | string | `ilp` | 排除算法（greedy/ilp/both） |
| `--objective` | string | `spr_only` | 优化目标（spr_only/multi_objective） |
| `--auto_bin` | flag | off | 启用信息增益自动分箱 |
| `--uplift` | flag | off | 启用Uplift因果推断分析 |
| `--stability_data` | string | - | 多期稳定性检验数据路径（逗号分隔） |
| `--backtest_dir` | string | - | 批量回测数据目录 |
...
### 0-1整数线性规划（ILP）求解器（v2.0新增）

使用PuLP/CBC求解全局最优排除方案，突破贪心算法的矩形约束：

- **决策变量**：144个二元变量（12×12网格每个格子是否排除）
- **目标函数**：最大化排除收益 Σ x[i,j] × (spr_threshold - spr[i,j]) × ato[i,j]
- **约束条件**：
  - SPR约束：排除区域整体SPR ≤ 阈值（线性化）
  - 体量约束：排除交易占比 ≤ 20%
  - 质量守卫：高SPR格子不排除
  - 非空约束：至少排除1个格子
- **优势**：全局最优、支持非矩形区域、毫秒级求解

### 阈值自适应调整

当约束条件无法满足时，使用二分搜索自动调整阈值：
- 搜索范围：[0, spr_threshold]
- 收敛条件：hi - lo < 0.001
- 提前终止：排除占比在约束限制的0.5%以内
...
- 响应式设计,支持移动端
- 交互式表格(排序、筛选)
- ECharts交互式热力图（鼠标悬停查看详情）
- 新旧策略差异对比热力图（置入/置出/保留/排除四色标注）
- 可编辑文本内容
- 单文件自包含,可离线使用
...
│   ├── place_in_out_algorithm.py  # 置入置出算法模块（贪心）
│   ├── ilp_solver.py           # ILP最优求解器模块
│   ├── report_generator.py     # Excel报告生成模块
│   ├── html_report_generator.py  # HTML报告生成模块（含ECharts）
│   ├── metrics.py              # 共享指标计算模块
│   ├── auto_binning.py         # 信息增益自动分箱
│   ├── stability.py            # PSI时序稳定性检验
│   ├── backtester.py           # 批量回测框架
│   ├── uplift_model.py         # Uplift因果推断框架
│   ├── utils.py                # 通用工具函数
...
- **数据处理速度**：向量化处理，10-50x faster than v1.x
- **ILP求解时间**：144变量毫秒级求解（CBC solver）
- **典型分析时间**：5-15秒（取决于数据量）
...
- 2.1.0 (2026-04-09)：6大新模块 + 全面集成
  - 共享指标模块 metrics.py，消除报告间重复计算
  - 多目标优化 (SPR+CPS Pareto前沿，epsilon-constraint方法)
  - PSI时序稳定性检验（多期策略漂移检测）
  - 信息增益自动分箱（替代固定20q→12Q映射）
  - 批量回测框架（多期数据自动策略对比+趋势分析）
  - Uplift因果推断框架（聚合级CATE+可选sklearn T-Learner）
  - 新增5个CLI参数：--objective, --auto_bin, --uplift, --stability_data, --backtest_dir
- 2.0.0 (2026-04-09)：算法引擎升级 + 性能优化 + 可视化增强
  - 新增0-1整数线性规划（ILP）求解器，全局最优排除方案
  - 新增`--algorithm`参数（greedy/ilp/both），默认使用ILP
  - 新增`--model_x`/`--model_y`参数，支持自定义模型名称
  - 阈值调整从线性降低改为二分搜索，收敛更快更精确
  - 数据预处理向量化，10-50x性能提升
  - HTML报告集成ECharts交互式热力图
  - 新增新旧策略差异对比热力图（置入/置出四色标注）
  - 新增PuLP依赖（可选，ILP求解器需要）
